...
function StatusPill({ ok, label }: { ok: boolean; label: string }) {
  return (
    <span className={`inline-flex min-w-0 items-center gap-2 rounded-full px-2.5 py-1 text-xs font-semibold ${ok ? 'bg-ok/10 text-emerald-200' : 'bg-danger/10 text-red-200'}`}>
...
function StatusRow({ label, value, icon }: { label: string; value: ReactNode; icon: ReactNode }) {
  return (
    <div className="flex min-w-0 items-center justify-between gap-3 border-b border-panel px-4 py-3 last:border-b-0">
      <div className="flex min-w-0 items-center gap-3 text-sm text-muted">
        <span className="text-accent">{icon}</span>
        <span className="truncate">{label}</span>
      </div>
      <div className="min-w-0 max-w-[58%] text-right text-sm font-medium text-white">{value}</div>
    </div>
  );
}
...
export function StatusPanel({ stats, wsStatus, t }: { stats: Stats | null; wsStatus: 'connecting' | 'connected' | 'disconnected'; t: Copy }) {
  ...
  return (
    <section className="min-w-0 rounded-lg border border-panel bg-surface shadow-panel">
        ...
        <StatusRow
          ...
          value={
            <div className="flex min-w-0 flex-col items-end gap-1">
              ...
              <span className="max-w-full truncate text-xs text-muted">{audio?.lavalink_uri ?? '...'}</span>
            </div>
          }
        />
      </div>
